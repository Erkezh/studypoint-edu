  ...
  code = code.replace(
    ...
    (m, before, setShow) => {
      ...
      if (m.includes('problems[currentIndex].correctAnswer')) {
        // Ищем поле с текстом вопроса (equation, question, problem и т.д.)
        const questionField = m.includes('.equation') ? 'equation' :
                             m.includes('.question') ? 'question' :
                             m.includes('.problem') ? 'problem' : 'equation'
        return before + setShow + `\n    const __isCorrect = userAnswer === problems[currentIndex].correctAnswer;\n    const __question = problems[currentIndex].${questionField} || problems[currentIndex].question || '';\n    window.parent.postMessage({ type: "exercise-result", id: null, correctAnswer: problems[currentIndex].correctAnswer, studentAnswer: userAnswer, userAnswer: userAnswer, isCorrect: __isCorrect, question: __question }, "*");`
      }
      ...
      if (correctMatch) {
        ...
        const questionField = m.includes('.equation') ? 'equation' :
                             m.includes('.question') ? 'question' :
                             m.includes('.problem') ? 'problem' : 'equation'
        return before + setShow + `\n    const __isCorrect = userAnswer === ${varName}[currentIndex].correctAnswer;\n    const __question = ${varName}[currentIndex].${questionField} || ${varName}[currentIndex].question || '';\n    window.parent.postMessage({ type: "exercise-result", id: null, correctAnswer: ${varName}[currentIndex].correctAnswer, studentAnswer: userAnswer, userAnswer: userAnswer, isCorrect: __isCorrect, question: __question }, "*");`
      }
      // Если не нашли, используем общий паттерн
      return before + setShow + '\n    const __isCorrect = userAnswer === correctAnswer;\n    window.parent.postMessage({ type: "exercise-result", id: null, correctAnswer: correctAnswer, studentAnswer: userAnswer, userAnswer: userAnswer, isCorrect: __isCorrect, question: "" }, "*");'
    }
  )
