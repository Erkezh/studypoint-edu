...
onMounted(() => {
  ...
  scene.background = new THREE.Color('#0a140b')
  scene.fog = new THREE.Fog('#0a140b', 9, 24)
...
function buildGarage(targetScene: THREE.Scene, targetPlatform: THREE.Group) {
  const ambient = new THREE.HemisphereLight('#d9ffd3', '#101708', 0.6)
  targetScene.add(ambient)

  const key = new THREE.DirectionalLight('#f3ffe8', 1.5)
  key.position.set(5, 6, 4)
  key.castShadow = true
  key.shadow.mapSize.set(2048, 2048)
  targetScene.add(key)

  const fill = new THREE.DirectionalLight('#9aff75', 0.42)
  fill.position.set(-4, 3, 2)
  targetScene.add(fill)

  const rim = new THREE.DirectionalLight('#57ff25', 0.34)
  ...
  targetScene.add(
    new THREE.Points(
      particles,
      new THREE.PointsMaterial({ color: '#b7ff8a', size: 0.018, transparent: true, opacity: 0.42 })
    )
  )
}
...
function buildPodium(targetPlatform: THREE.Group) {
  ...
  const accent = new THREE.MeshBasicMaterial({ color: '#72ff20' })

  const ground = new THREE.Mesh(
    new THREE.CircleGeometry(5.5, 64),
    new THREE.MeshStandardMaterial({ color: '#101910', metalness: 0.38, roughness: 0.58 })
  )
  ...
  const underGlow = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: '#72ff20', transparent: true, opacity: 0.42 })
  )
...
function setupGarageMaterial(material: THREE.Material, meshName: string) {
  ...
  material.emissiveMap?.colorSpace && (material.emissiveMap.colorSpace = THREE.SRGBColorSpace)
...
function makeFallbackCar() {
  ...
  const material = new THREE.MeshStandardMaterial({ color: props.paintColor ?? '#0b3a31', metalness: 0.45, roughness: 0.28 })
  const body = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.72, 5), material)
