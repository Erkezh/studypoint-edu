  ...
  <header class="shadow-md relative z-50" style="background-color: #38B000;">
    <nav class="container mx-auto px-4 py-3">
      <div class="flex items-center justify-between">
        <!-- Logo -->
        <router-link to="/" @click="onLogoClick" class="flex items-center shrink-0 cursor-pointer">
          <span class="text-white font-bold text-xl hover:opacity-90 transition-opacity cursor-pointer">StudyPoint</span>
        </router-link>
        ...
        <div class="hidden md:flex items-center gap-6">
          ...
          <template v-else>
            ...
            <template v-else>
              ...
              <button v-if="authStore.user?.role === 'STUDENT'" type="button" class="text-white hover:text-gray-100 transition-colors font-medium" @click="openGameSelection()">{{ activeGameNavLabel }}</button>
            </template>
            <router-link to="/analytics" class="text-white hover:text-gray-100 transition-colors font-medium">Талдау</router-link>
        ...
        <div class="flex items-center gap-3">
          ...
          <div v-if="authStore.isAuthenticated" class="relative hidden md:block" ref="profileDropdownRef">
            ...
            <div v-if="showProfileMenu" class="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl py-2 z-50 border border-gray-100 overflow-hidden">
              ...
              <button v-if="authStore.user?.role === 'STUDENT'" type="button" @click="openGameSelection('profile')"
                class="flex w-full items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-gray-50 transition-colors">
                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="showMobileMenu" class="md:hidden mt-4 pb-4 border-t border-white/20 pt-4 space-y-3">
          ...
          <template v-else>
            ...
            <nav class="space-y-1">
              ...
              <router-link v-if="authStore.user?.role === 'TEACHER' || authStore.user?.role === 'ADMIN'" to="/topics" @click="showMobileMenu = false" class="flex items-center gap-3 px-3 py-2.5 text-white hover:bg-white/10 rounded-xl transition-all font-medium">
                <svg class="w-5 h-5 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                Оқу
              </router-link>
              <router-link v-else-if="userGradeLevel" :to="{ name: 'class', params: { gradeId: userGradeLevel } }" @click="showMobileMenu = false" class="flex items-center gap-3 px-3 py-2.5 text-white hover:bg-white/10 rounded-xl transition-all font-medium">
                <svg class="w-5 h-5 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                Оқу
              </router-link>
              <router-link to="/analytics" @click="showMobileMenu = false" class="flex items-center gap-3 px-3 py-2.5 text-white hover:bg-white/10 rounded-xl transition-all font-medium">
              ...
              <button v-if="authStore.user?.role === 'STUDENT'" type="button" @click="openGameSelection('mobile')" class="flex w-full items-center gap-3 px-3 py-2.5 text-white hover:bg-white/10 rounded-xl transition-all font-medium">
