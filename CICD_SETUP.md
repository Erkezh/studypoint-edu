...
`studypoint-frontend.service` must run with strict port:
...
- `DEPLOY_SSH_HOST` = server IP/host (example: `49.12.99.145`)
- `DEPLOY_SSH_PORT` = `22`
- `DEPLOY_SSH_USER` = `studypoint`
- `DEPLOY_SSH_PRIVATE_KEY` = full content of `~/.ssh/studypoint_actions` (private key)
- `DEPLOY_PATH` = `/studypoint-edu/studypoint-edu` (optional but recommended)
- `DEPLOY_DOMAIN` = `edu.studypoint.kz` (optional)
...
git commit -m "Add GitHub Actions production deploy pipeline"
...
- Do not run `nohup npm run preview` manually after this; use systemd service only.
